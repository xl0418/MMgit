...
p2 = as.data.frame(p2)
 plot_p <- ggplot(p2, aes(x=factor(group), y=p)) +geom_boxplot()+xlab("Mig rate")+ylab("p-value")+#geom_hline(yintercept = 0.05)
...
plot_power <- ggplot(p2, aes(x=factor(group), y=power)) +geom_boxplot()+xlab("Mig rate")+ylab("power of test")#geom_hline(yintercept = 0.05)
 # geom_hline(aes(yintercept=0.05)) + geom_text(aes(0,0.05,label = 0.05, vjust = 0))
# + geom_dotplot(binaxis='y', stackdir='center', dotsize=1,binwidth = )
# p <- ggplot(p2, aes(x=factor(group), y=power)) +geom_boxplot()
plot_power
